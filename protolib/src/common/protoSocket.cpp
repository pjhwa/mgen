...
{
#ifndef SO_BROADCAST
    PLOG(PL_ERROR, "ProtoSocket::SetBroadcast(): SO_BROADCAST not defined\n");
    return false;
#else
#ifdef WIN32
...
#endif
    PLOG(PL_DEBUG, "Setting SO_BROADCAST to %d on socket %d\n", state, handle);
    if (setsockopt(handle, SOL_SOCKET, SO_BROADCAST, (char*)&state, sizeof(state)) < 0)
    {
        PLOG(PL_ERROR, "setsockopt(SO_BROADCAST) failed: %s\n", strerror(errno));
        return false;
    }
    PLOG(PL_DEBUG, "SO_BROADCAST set successfully\n");
#endif
    return true;
}
